...
+------------------------------+-----------------+------------------+----------------------+-----------------------+---------------------+---------------------------+
| Dot density                  | `tc_map_dots()` | stock            | ![](fig/poly_dd.svg) |                       |                     | `dotDensityLayer()`       |
+------------------------------+-----------------+------------------+----------------------+-----------------------+---------------------+---------------------------+
| Discontinuities              | `tc_map_disc()` | stock \| ratio   | ![](fig/poly_d.svg)  |                       |                     | `discLayer()`             |
+------------------------------+-----------------+------------------+----------------------+-----------------------+---------------------+---------------------------+
...
`tc_init()` is designed to initiate a map centered on a specific extent (`x` bbox). The `theme` argument allow to select a theme and the `shadow` argument add a shadow.

`tc_map()` displays `sf` objects geometries.

`tc_layout()` add a complete layout to the map (title, credits, north arrow, scale bar). Each layout elements can also be set separetly (with `tc_title()`, `tc_credits()`, `tc_arrow()` and `tc_scale()`).
...
`tc_map_p()` displays symbols with areas proportional to a quantitative variable (stocks). 
The `inches` argument is used to customize the symbols sizes.
...
`tc_map_pc()` creates a map of symbols that are proportional to values of a first variable and colored to reflect the classification of a second variable. A combination of `tc_map_p()` and `tc_map_c()` arguments is used.
